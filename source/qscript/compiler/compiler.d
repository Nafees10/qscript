...
import qscript.qscript : QScriptBytecode;
...
public struct CompileError{
	...
	size_t lineno; /// The line number on which the error is
	/// constructor
	this(size_t lineNumber, string errorMessage){
		lineno = lineNumber;
		msg = errorMessage;
	}
}
...
public struct Struct{
	...
	string fromString(string str){
		...
		name = vals[1];
		vals = vals[2 .. $];
		immutable size_t dTypeStartIndex = vals.length/2;
...
public struct DataType{
	...
	this (DataType.Type dataType, size_t arrayDimension = 0, bool isReference = false){
		type = dataType;
	...
	/// arrayDimension is the number of nested arrays
	/// isRef is whether the type is a reference to the actual type
	this (string dataType, size_t arrayDimension = 0, bool isReference = false){
		this.name = dataType;
		arrayDimensionCount = arrayDimension;
		isRef = isReference;
	}
	...
	void fromString(string s){
		...
		// now read the index
		for (size_t i = sType.length; i < s.length; i ++){
			if (s[i] == '['){
